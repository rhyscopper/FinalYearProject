// a simple SVM classifier (a wrapper for LIBLINEAR)
...
public class SVMClassifier {
  ...
  public double[] predict(String sentence) {
    ...
    for (int i=0; i<models.length; i++) {
      ...
      System.out.println(i + " " + emotions[i] + " res " + r + " raw " + Arrays.toString(val));
...
//      System.out.println("fn " + i + "=" + fn[i]);
  ...
  // train new models
  public int train() {
    Model[] newmodels = new Model[emotions.length];
...
//    }
    ...
    featureMap.clear(); 
    for (String emotion : emotions) {
      List<String> lines = Utils.readFileLines(installDir + File.separator + trainDir + File.separator + emotion + ".txt");
...
//        }
    ...
    writeFeatureMap(installDir + File.separator + trainDir + File.separator + featureFile);
...
//      System.out.println("test: " + r + " " + Arrays.toString(val));
      try {
        model.save(new File(installDir + File.separator + trainDir + File.separator + emotion + "-model"));
      } catch (IOException e) {
        e.printStackTrace();
        System.exit(0);
      }
